...
public abstract class AbstractQueryBuilder<ENTITY, FILTER extends SearchFilter> {
    ...
    public List<ENTITY> selectAllByFilters(FILTER filters) {
        ...
        if (!filters.getPagination().getNoPagination()) {
            if (filters.getPagination().getPageNum() != null)
                query.setFirstResult(filters.getPagination().getPageNum() * filters.getPagination().getPageSize());
            else if (filters.getPagination().getPageOffset() != null)
                query.setFirstResult(filters.getPagination().getPageOffset());
            query.setMaxResults(filters.getPagination().getPageSize());
        }
    ...
    protected String constructOrderStatement(FILTER filters) {
        ...
        String dir = (filters.getSorting().getDir() == null || filters.getSorting().getDir().isBlank())
                ? "ASC" : filters.getSorting().getDir();
